...
class SpeakerInferenceService:
    ...
    def predict_speaker(self, audio_path: Path) -> Dict[str, object]:
        ...
        Returns:
            ...
            - 'probabilities': Dict of {speaker: confidence} (Top 5)
        """
        self._load_artifacts()
        
        # Extract embeddings using XLSR-53 with custom pooling (mean + std from layers 6-13)
        # This matches the training logic for the 40-speaker model
        ...
        if hasattr(self._classifier, "predict_proba"):
            probs = self._classifier.predict_proba(embedding_pca)[0]
        elif hasattr(self._classifier, "decision_function"):
            decision = self._classifier.decision_function(embedding_pca)[0]
        ...
        probabilities = {lbl: prob for lbl, prob in top_5}
