...
class IntentInferenceService:
    ...
    @property
    def extractor(self):
        global WavLMFeatureExtractor
        if self._extractor is None:
            if WavLMFeatureExtractor is None:
            ...
            self._extractor = WavLMFeatureExtractor(model_name="microsoft/wavlm-base")
        return self._extractor

    def _load_intent_artifacts(self):
        if self._classifier is not None: return
        import joblib
        model_path = MODELS_DIR / "intent_model_svm.pkl"
    ...
    def predict_intent(self, audio_path):
        ...
        emb = self.extractor.extract_from_file(str(audio_path)).reshape(1, -1)
        emb_scaled = self._scaler.transform(emb)

        pred_idx = int(self._classifier.predict(emb_scaled)[0])
        reverse_encoder = {v: k for k, v in self._encoder.items()}
        pred_label = reverse_encoder[pred_idx]

        #pred_label = self._encoder[pred_idx]

        probs = self._classifier.predict_proba(emb_scaled)[0]

        # FIXED: get class labels from dict values
        labels = list(self._encoder.values())
...
class EmotionInferenceService:
    ...
    def predict_emotion(self, audio_path: Path) -> Dict[str, object]:
        ...
        if hasattr(self._classifier, "predict_proba"):
            ...
            probs = np.ones(len(self.encoder.classes)) / len(self.encoder.classes)

        # Return probabilities as dict {label: prob}
        labels = list(self.encoder.classes)
        prob_map = {str(lbl): float(probs[i]) for i, lbl in enumerate(labels)}
...
def create_app() -> Flask:
    ...
    def intent_page():
        result = session.get("intent_result")
    ...
    def intent_predict():
        ...
        print("DEBUG: tmp_path =", tmp_path, "exists:", tmp_path.exists())

        try:
            # ---- extra guards so we get a CLEARER error ----
            if tmp_path is None:
                raise ValueError("tmp_path is None before prediction")
            ...
            import traceback
            print("\n===== INTENT PREDICTION ERROR TRACEBACK =====")
            traceback.print_exc()
            print("=============================================\n")
            ...
            if tmp_path is not None:
                try:
                    ...
                    print("WARNING: failed to delete tmp_path:", del_e)
